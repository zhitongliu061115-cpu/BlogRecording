...
- [ ] 3.3 Replace local placeholder AI replies with existing DeepSeek session QA calls; verify persisted QA messages are shown on AI and detail flows.
- [ ] 3.4 Show answering, failed, missing API key, empty content, and retry states on the AI page using the same QA status semantics as detail.
...
- [ ] 5.3 Run targeted unit tests for UI navigation, AI chat state, and QA reuse.

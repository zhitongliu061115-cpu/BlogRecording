...
- [ ] 1.1 Run and record `git status --short`, `git diff --check`, and `openspec.cmd status --change "refactor-sdd-harness-alignment" --json`.
- [ ] 1.2 Run current `.\gradlew.bat testDebugUnitTest` and record failures or environment blockers before code changes.
